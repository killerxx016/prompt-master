...
const AUTH_USERS_KEY = 'pm-auth-users';
...
function AuthScreen({ onAuthenticated }) {
  ...
  const handleCreateAccount = (event) => {
    ...
    const usernameKey = normalizeAuthValue(cleanUsername);
    const emailKey = normalizeAuthValue(cleanEmail);
    const existingUser = users.find((user) => (
      normalizeAuthValue(user.username) === usernameKey
      || normalizeAuthValue(user.email) === emailKey
    ));
    ...
    writeStoredUsers([user, ...users]);
    setMessage(`Account created for ${user.username}. Please log in to continue.`);
  ...
  const handleLogin = (event) => {
    ...
    setError('');
    setMessage('');
    const cleanIdentifier = normalizeAuthValue(identifier);
    ...
    const user = readStoredUsers().find((storedUser) => (
      (normalizeAuthValue(storedUser.email) === cleanIdentifier
        || normalizeAuthValue(storedUser.username) === cleanIdentifier)
  ...
  return (
    <main className="auth-page">
      <section className="auth-window">
        <div className="auth-brand">Prompt Master</div>
        ...
        <p className="auth-note">Accounts are stored only in this browser with localStorage.</p>
      </section>
    </main>
  );
}
...
const SUGGESTION_CATEGORIES = Array.from(new Set(IDEA_SUGGESTIONS.map((item) => item.category)));
...
function localizedSuggestion(item, language) {
  ...
  const type = translateType(item.type, language);
  ...
  if (language === 'tr') {
    return {
      ...
      typeLabel: type,
      titleLabel: localeEntry?.title || type,
      platformLabel: item.platform,
      tagsLabel: item.tags.map((tag) => tag),
      useCaseLabel: localeEntry?.useCase || `${category} kategorisinde "${type}" amac\u0131yla haz\u0131r ve do\u011frudan kullan\u0131labilir bir prompt.`,
      promptTextLabel: localeEntry?.promptText || `${type} i\u00e7in profesyonel, net ve uygulanabilir bir prompt haz\u0131rla.\n\nKategori: ${category}\nHedef: Kullan\u0131c\u0131n\u0131n bu fikri kolayca uygulayabilmesi ve kaliteli bir sonu\u00e7 almas\u0131.\n\n\u0130stenen sonu\u00e7:\n1. Kullan\u0131c\u0131dan gerekli ba\u011flam\u0131 iste.\n2. Ana hedefi, k\u0131s\u0131tlar\u0131 ve kalite beklentilerini netle\u015ftir.\n3. Uygulanacak ad\u0131mlar\u0131 sade ve net yaz.\n4. Sonucun hangi formatta verilmesi gerekti\u011fini belirt.\n5. Gereksiz ayr\u0131nt\u0131 ekleme; kaliteli, modern ve kullan\u0131ma haz\u0131r bir \u00e7\u0131kt\u0131 \u00fcret.`,
    };
  }
  ...
  if (language === 'ar') {
    return {
      ...
      platformLabel: item.platform,
      tagsLabel: item.tags.map((tag) => tag),
      useCaseLabel: localeEntry?.useCase || `مطالبة جاهزة ومباشرة ضمن فئة ${category} لاستخدام ${type}.`,
      promptTextLabel: localeEntry?.promptText || `اكتب مطالبة احترافية وواضحة لاستخدام ${type}.\n\nالفئة: ${category}\nالهدف: مساعدة المستخدم على تطبيق الفكرة بسهولة والحصول على نتيجة عالية الجودة.\n\nالمطلوب:\n1. اطلب من المستخدم السياق الضروري.\n2. وضّح الهدف الرئيسي والقيود ومعايير الجودة.\n3. اكتب خطوات التنفيذ بطريقة بسيطة وواضحة.\n4. حدّد شكل النتيجة النهائية المطلوبة.\n5. لا تضف تفاصيل غير ضرورية؛ اجعل الناتج حديثا، منظما، وجاهزا للاستخدام.`,
    };
  }
...
export default function Home() {
  ...
  const visibleCategories = SUGGESTION_CATEGORIES.map((category) => ({
    name: category,
    count: filteredSuggestions.filter((item) => item.category === category).length,
  })).filter((category) => category.count > 0 || !librarySearchQuery.trim());
  ...
  const categorySuggestions = filteredSuggestions.filter((item) => item.category === activeCategory);
    ...
    try {
      ...
      if (savedSession) {
        ...
        const matchingUser = readStoredUsers().find((user) => (
          user.id === parsedSession.id
          && normalizeAuthValue(user.username) === normalizeAuthValue(parsedSession.username)
          && normalizeAuthValue(user.email) === normalizeAuthValue(parsedSession.email)
        ));
      ...
      const saved = localStorage.getItem('pm-history');
  ...
  const saveToHistory = (inputText, resultText, selectedMode) => {
    const entry = {
      id: Date.now(),
      input: inputText,
      result: resultText,
  ...
  const handleEnhance = async () => {
    ...
    try {
      const res = await fetch('/api/enhance', {
        ...
        body: JSON.stringify({ text: input, mode, language }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Something went wrong');
      setResult(data.result);
      if (!temporaryChat) saveToHistory(input, data.result, mode);
    } catch (err) {
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '48px 24px', display: 'flex', flexDirection: 'column', gap: 32 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <button
            ...
            onMouseEnter={(e) => { if (!loading && input.trim()) e.target.style.transform = 'translateY(-1px)'; }}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: 10 }}>
                      {categorySuggestions.map((item) => {
                        ...
                        <button
                          key={item.id}
                          onClick={() => {
                            setSelectedSuggestionCategory(item.category);
