...
export class clsUserBusiness {
  ...
  static async LoginUser({ email, password }) {
    if (!email?.trim()) throw new Error('Email or username is required');
    ...
    if (!resolvedEmail.includes('@')) {
      let userRecord;
      try {
        userRecord = await clsUserData.FindByUsername(email.trim());
      } catch (_) {
    ...
    return clsUserData.Login({ email: resolvedEmail, password });
  }
